...
template<match_type_t MATCH_TYPE, class T>
constexpr auto aksdjakl() {
    if constexpr (MATCH_TYPE == match_type_t::MATCHEQUALTO) return std::equal_to<T>();
    if constexpr (MATCH_TYPE == match_type_t::MATCHNOTEQUALTO) return std::not_equal_to<T>();
    if constexpr (MATCH_TYPE == match_type_t::MATCHGREATERTHAN) return std::greater<T>();
    if constexpr (MATCH_TYPE == match_type_t::MATCHLESSTHAN) return std::less<T>();

    if constexpr (MATCH_TYPE == match_type_t::MATCHNOTCHANGED) return std::equal_to<T>();
    if constexpr (MATCH_TYPE == match_type_t::MATCHCHANGED) return std::not_equal_to<T>();
    if constexpr (MATCH_TYPE == match_type_t::MATCHINCREASED) return std::greater<T>();
    if constexpr (MATCH_TYPE == match_type_t::MATCHDECREASED) return std::less<T>();

    if constexpr (MATCH_TYPE == match_type_t::MATCHINCREASEDBY) return std::minus<T>();
    if constexpr (MATCH_TYPE == match_type_t::MATCHDECREASEDBY) return std::plus<T>();
...
constexpr void flag_shrink(flag& f, size_t size) {
    if UNLIKELY(size <= 7) f &= ~flag_t::n64;
    if UNLIKELY(size <= 6) f &= ~flag_t::n64;
    if UNLIKELY(size <= 5) f &= ~flag_t::n64;
    if UNLIKELY(size <= 4) f &= ~flag_t::n64;
    if UNLIKELY(size <= 3) f &= ~flag_t::n32;
    if UNLIKELY(size <= 2) f &= ~flag_t::n32;
    if UNLIKELY(size <= 1) f &= ~flag_t::n16;
    if UNLIKELY(size == 0) f = flag_t::none;
}


template<match_type_t MATCH_TYPE, flag_t TYPE_SPEC>
flag scan_routine(const a64 *mem,
    ...
    if constexpr (MATCH_TYPE == match_type_t::MATCHANY
                  || MATCH_TYPE == match_type_t::MATCHUPDATE) {
        flag ret{user_value.vars[0].type};
        flag_shrink(ret, size);
        return ret;
    }
    if constexpr (MATCH_TYPE == match_type_t::MATCHEQUALTO
                  || MATCH_TYPE == match_type_t::MATCHNOTEQUALTO
                  || MATCH_TYPE == match_type_t::MATCHGREATERTHAN
                  || MATCH_TYPE == match_type_t::MATCHLESSTHAN) {
        flag ret{user_value.vars[0].type};
        flag_shrink(ret, size);
        if constexpr (TYPE_SPEC & flag_t::i8 ) if ((user_value.vars[0].type & flag_t::i8 ) == flag_t::none || !aksdjakl<MATCH_TYPE, ::u8 >()(mem->u8 , user_value.vars[0].u8 )) { ret &= ~flag_t::i8 ; }
        if constexpr (TYPE_SPEC & flag_t::i16) if ((user_value.vars[0].type & flag_t::i16) == flag_t::none || !aksdjakl<MATCH_TYPE, ::u16>()(mem->u16, user_value.vars[0].u16)) { ret &= ~flag_t::i16; }
        if constexpr (TYPE_SPEC & flag_t::i32) if ((user_value.vars[0].type & flag_t::i32) == flag_t::none || !aksdjakl<MATCH_TYPE, ::u32>()(mem->u32, user_value.vars[0].u32)) { ret &= ~flag_t::i32; }
        if constexpr (TYPE_SPEC & flag_t::i64) if ((user_value.vars[0].type & flag_t::i64) == flag_t::none || !aksdjakl<MATCH_TYPE, ::u64>()(mem->u64, user_value.vars[0].u64)) { ret &= ~flag_t::i64; }
        // if constexpr (TYPE_SPEC & flag_t::f32) if ((user_value.vars[0].type & flag_t::f32) == flag_t::none || !aksdjakl<MATCH_TYPE, ::f32>()(mem->f32, user_value.vars[0].f32)) { ret &= ~flag_t::f32; }
        // if constexpr (TYPE_SPEC & flag_t::f64) if ((user_value.vars[0].type & flag_t::f64) == flag_t::none || !aksdjakl<MATCH_TYPE, ::f64>()(mem->f64, user_value.vars[0].f64)) { ret &= ~flag_t::f64; }
        // std::cout<<"ret: "<<ret<<'\n';
        return ret;
    }
    // if constexpr (MATCH_TYPE == match_type_t::MATCHNOTCHANGED
    //               || MATCH_TYPE == match_type_t::MATCHCHANGED
    //               || MATCH_TYPE == match_type_t::MATCHINCREASED
    //               || MATCH_TYPE == match_type_t::MATCHDECREASED) {
    //     flag ret{old_match->type};
    //     flag_shrink(ret, size);
    //     if constexpr (TYPE_SPEC & flag_t::u8 ) if (user_value.vars[0].type & flag_t::u8  && !aksdjakl<MATCH_TYPE>(mem->u8 , old_match->value.u8 )) { ret &= flag_t::u8 ; }
    //     if constexpr (TYPE_SPEC & flag_t::u16) if (user_value.vars[0].type & flag_t::u16 && !aksdjakl<MATCH_TYPE>(mem->u16, old_match->value.u16)) { ret &= flag_t::u16; }
    //     if constexpr (TYPE_SPEC & flag_t::u32) if (user_value.vars[0].type & flag_t::u32 && !aksdjakl<MATCH_TYPE>(mem->u32, old_match->value.u32)) { ret &= flag_t::u32; }
    //     if constexpr (TYPE_SPEC & flag_t::u64) if (user_value.vars[0].type & flag_t::u64 && !aksdjakl<MATCH_TYPE>(mem->u64, old_match->value.u64)) { ret &= flag_t::u64; }
    //     if constexpr (TYPE_SPEC & flag_t::s8 ) if (user_value.vars[0].type & flag_t::s8  && !aksdjakl<MATCH_TYPE>(mem->s8 , old_match->value.s8 )) { ret &= flag_t::s8 ; }
    //     if constexpr (TYPE_SPEC & flag_t::s16) if (user_value.vars[0].type & flag_t::s16 && !aksdjakl<MATCH_TYPE>(mem->s16, old_match->value.s16)) { ret &= flag_t::s16; }
    //     if constexpr (TYPE_SPEC & flag_t::s32) if (user_value.vars[0].type & flag_t::s32 && !aksdjakl<MATCH_TYPE>(mem->s32, old_match->value.s32)) { ret &= flag_t::s32; }
    //     if constexpr (TYPE_SPEC & flag_t::s64) if (user_value.vars[0].type & flag_t::s64 && !aksdjakl<MATCH_TYPE>(mem->s64, old_match->value.s64)) { ret &= flag_t::s64; }
    //     if constexpr (TYPE_SPEC & flag_t::f32) if (user_value.vars[0].type & flag_t::f32 && !aksdjakl<MATCH_TYPE>(mem->f32, old_match->value.f32)) { ret &= flag_t::f32; }
    //     if constexpr (TYPE_SPEC & flag_t::f64) if (user_value.vars[0].type & flag_t::f64 && !aksdjakl<MATCH_TYPE>(mem->f64, old_match->value.f64)) { ret &= flag_t::f64; }
    //     return ret;
    // }
    // if constexpr (MATCHTYPE == match_type_t::MATCHINCREASEDBY
    //               || MATCHTYPE == match_type_t::MATCHDECREASEDBY) {
    //     flag ret{flag_t::none};
    //     if (old_match.type & flag::s_from_size(DATAWIDTH) && user_value.vars[0].type & flag::s_from_size(DATAWIDTH) && s_asdasd<DATAWIDTH, MATCHTYPE>(mem, old_match, &user_value.vars[0])) { ret |= flag::s_from_size(DATAWIDTH); }
    //     if (old_match.type & flag::u_from_size(DATAWIDTH) && user_value.vars[0].type & flag::u_from_size(DATAWIDTH) && u_asdasd<DATAWIDTH, MATCHTYPE>(mem, old_match, &user_value.vars[0])) { ret |= flag::u_from_size(DATAWIDTH); }
    //     if (old_match.type & flag::f_from_size(DATAWIDTH) && user_value.vars[0].type & flag::f_from_size(DATAWIDTH) && f_asdasd<DATAWIDTH, MATCHTYPE>(mem, old_match, &user_value.vars[0])) { ret |= flag::f_from_size(DATAWIDTH); }
    //     return ret;
    // }
    ...
    //     return ret;
    // }
    // if constexpr (MATCHTYPE == match_type_t::MATCHEXCLUDE) {
    //     if (user_value.vars[0].type & flag::s_from_size(DATAWIDTH) && i_asdasd<DATAWIDTH, std::less_equal>(mem, old_match)) { ret |= flag::s_from_size(DATAWIDTH); }
    //     //...
    //     return ret;
    // }
    return flag{flag_t::none};
...
//todo[med] #1005 move to another header
...
// }
//
// static inline void fix_endianness(PWNER::value_t *data_value, bool reverse_endianness)
// {
//     if (!reverse_endianness) {
//         return;
//     }
//     if (data_value->flags & PWNER::flag_t::flags_64b) {
//         data_value->u64 = swap_bytes64(data_value->u64);
//     } else if (data_value->flags & PWNER::flag_t::flags_32b) {
//         data_value->u32 = swap_bytes32(data_value->u32);
//     } else if (data_value->flags & PWNER::flag_t::flags_16b) {
...
//             saveflags |= PWNER::flag_t::flag_i##DATAWIDTH; \
//             saveflags |= PWNER::flag_t::flag_u##DATAWIDTH; \
//             return (DATAWIDTH)/8; \
//         } \
//         else { \
//             return 0; \
...
//         int ret = 0; \
//         if (old_value->flags & PWNER::flag_t::flag_i##DATAWIDTH) { ret = (DATAWIDTH)/8; saveflags |= PWNER::flag_t::flag_i##DATAWIDTH; } \
//         if (old_value->flags & PWNER::flag_t::flag_u##DATAWIDTH) { ret = (DATAWIDTH)/8; saveflags |= PWNER::flag_t::flag_u##DATAWIDTH; } \
//         return ret; \
//     }
//
...
//     { \
//         if (memlength >= (DATAWIDTH)/8) { \
//             saveflags |= PWNER::flag_t::flag_f##DATAWIDTH; \
//             return (DATAWIDTH)/8; \
//         } \
//         else { \
//             return 0; \
...
//         if (memlength < (DATAWIDTH)/8) return 0; \
//         int ret = 0; \
//         if (old_value->flags & PWNER::flag_t::flag_f##DATAWIDTH) { ret = (DATAWIDTH)/8; saveflags |= PWNER::flag_t::flag_f##DATAWIDTH; } \
//         return ret; \
//     }
//
...
//         if (memlength >= (DATAWIDTH)/8) return 0; \
//         int ret = 0; \
//         if (old_value->flags & PWNER::flag_t::flag_i##DATAWIDTH && user_value->flags & PWNER::flag_t::flag_i##DATAWIDTH \
//         &&  memory_ptr->i##DATAWIDTH == old_value->i##DATAWIDTH OP user_value->i##DATAWIDTH) \
//             { ret = (DATAWIDTH)/8; saveflags |= PWNER::flag_t::flag_i##DATAWIDTH; } \
//         if (old_value->flags & PWNER::flag_t::flag_u##DATAWIDTH && user_value->flags & PWNER::flag_t::flag_u##DATAWIDTH \
//         &&  memory_ptr->u##DATAWIDTH == old_value->u##DATAWIDTH OP user_value->u##DATAWIDTH) \
//             { ret = (DATAWIDTH)/8; saveflags |= PWNER::flag_t::flag_u##DATAWIDTH; } \
//         return ret; \
//     }
//
...
//         if (memlength < (DATAWIDTH)/8) return 0; \
//         int ret = 0; \
//         if (old_value->flags & PWNER::flag_t::flag_f##DATAWIDTH && user_value->flags & PWNER::flag_t::flag_f##DATAWIDTH \
//         &&  memory_ptr->f##DATAWIDTH == old_value->f##DATAWIDTH OP user_value->f##DATAWIDTH) \
//             { ret = (DATAWIDTH)/8; saveflags |= PWNER::flag_t::flag_f##DATAWIDTH; } \
//         return ret; \
//     }
//
...
//         } \
//         if ((memlength >= (DATAWIDTH)/8) \
//                 && (user_value[0].flags & PWNER::flag_t::flag_i##DATAWIDTH) \
//                 && (memory_ptr->i##DATAWIDTH >= user_value[0].i##DATAWIDTH) \
//                 && (memory_ptr->i##DATAWIDTH <= user_value[1].i##DATAWIDTH)) \
//             { ret = (DATAWIDTH)/8; saveflags |= PWNER::flag_t::flag_i##DATAWIDTH; } \
//         if ((memlength >= (DATAWIDTH)/8) \
//                 && (user_value[0].flags & PWNER::flag_t::flag_u##DATAWIDTH) \
...
//         } \
//         if ((memlength >= (DATAWIDTH)/8) \
//                 && (user_value[0].flags & PWNER::flag_t::flag_f##DATAWIDTH) \
...
// {
//     saveflags = (PWNER::flag_t)(MIN(memlength, (uint16_t)(-1)));
//     return memlength;
// }
//
...
//     saveflags = (PWNER::flag_t)(memlength);
//     return memlength;
// }
//
// /*---------------*/
...
//     const PWNER::wildcard_t *wildcards_array = user_value->wildcard_value;
...
//     {
//         if (*((uint64_t*)(bytes_array+i)) != (((PWNER::mem64_t*)(memory_ptr->bytes+i))->u64 & *((uint64_t*)(wildcards_array+i))))
//         {
//             /* not matched */
//             return 0;
//         }
...
//         {
//             if ((bytes_array+i)[j] != (((PWNER::mem64_t*)(memory_ptr->bytes+i))->bytes[j] & (wildcards_array+i)[j]))
//             {
//                 /* not matched */
//                 return 0;
//             }
...
//         { \
//             /* matched */ \
//             saveflags = (PWNER::flag_t)((WIDTH)/8); \
//             return (WIDTH)/8; \
//         } \
//         else \
//         { \
...
//     {
//         if(((PWNER::mem64_t*)(memory_ptr->chars+i))->i64 != *((int64_t*)(scan_string+i)))
//         {
//             /* not matched */
//             return 0;
//         }
...
//         for(j = 0; j < length - i; ++j)
//         {
//             if(((PWNER::mem64_t*)(memory_ptr->chars+i))->chars[j] != (scan_string+i)[j])
//             {
//                 /* not matched */
//                 return 0;
//             }
...
//     if ((dt == PWNER::Edata_type::SCANDATATYPE) && (mt == PWNER::Ematch_type::SCANMATCHTYPE)) \
//     { \
...
predicate_t PWNER::SCANNER::get_predicate(match_type_t mt,
                                          const user_value& user_value) {
    // return scan_routine<match_type_t::MATCHANY, flag_t::a64>;
    // return scan_routine<match_type_t::MATCHEQUALTO, flag_t::a64>;
    return scan_routine<match_type_t::MATCHEQUALTO, flag_t::i16>;
    // if (mt == match_type_t::MATCHANY     && user_value.vars[0].type & flag_t::s8) return scan_routine<match_type_t::MATCHANY, flag_t::s8>;
    // if (mt == match_type_t::MATCHEQUALTO && user_value.vars[0].type & flag_t::s8) return scan_routine<match_type_t::MATCHANY, flag_t::s8>;
    ...
    // if ((dt == PWNER::data_type_t::INTEGER8) && (mt == PWNER::match_type_t::MATCHEQUALTO)) { return scan_routine_INTEGER8_EQUALTO; }
    // if ((dt == PWNER::data_type_t::INTEGER16) && (mt == PWNER::match_type_t::MATCHEQUALTO)) { if (reverse_endianness) { return scan_routine_INTEGER16_EQUALTO_REVENDIAN; } else { return scan_routine_INTEGER16_EQUALTO; }}
    // if ((dt == PWNER::data_type_t::INTEGER32) && (mt == PWNER::match_type_t::MATCHEQUALTO)) { if (reverse_endianness) { return scan_routine_INTEGER32_EQUALTO_REVENDIAN; } else { return scan_routine_INTEGER32_EQUALTO; }}
    // if ((dt == PWNER::data_type_t::INTEGER64) && (mt == PWNER::match_type_t::MATCHEQUALTO)) { if (reverse_endianness) { return scan_routine_INTEGER64_EQUALTO_REVENDIAN; } else { return scan_routine_INTEGER64_EQUALTO; }}
    // if ((dt == PWNER::data_type_t::FLOAT32) && (mt == PWNER::match_type_t::MATCHEQUALTO)) { if (reverse_endianness) { return scan_routine_FLOAT32_EQUALTO_REVENDIAN; } else { return scan_routine_FLOAT32_EQUALTO; }}
    // if ((dt == PWNER::data_type_t::FLOAT64) && (mt == PWNER::match_type_t::MATCHEQUALTO)) { if (reverse_endianness) { return scan_routine_FLOAT64_EQUALTO_REVENDIAN; } else { return scan_routine_FLOAT64_EQUALTO; }}
    // if ((dt == PWNER::data_type_t::ANYINTEGER) && (mt == PWNER::match_type_t::MATCHEQUALTO)) { if (reverse_endianness) { return scan_routine_ANYINTEGER_EQUALTO_REVENDIAN; } else { return scan_routine_ANYINTEGER_EQUALTO; }}
    // if ((dt == PWNER::data_type_t::ANYFLOAT) && (mt == PWNER::match_type_t::MATCHEQUALTO)) { if (reverse_endianness) { return scan_routine_ANYFLOAT_EQUALTO_REVENDIAN; } else { return scan_routine_ANYFLOAT_EQUALTO; }}
    // if ((dt == PWNER::data_type_t::ANYNUMBER) && (mt == PWNER::match_type_t::MATCHEQUALTO)) { if (reverse_endianness) { return scan_routine_ANYNUMBER_EQUALTO_REVENDIAN; } else { return scan_routine_ANYNUMBER_EQUALTO; }}
