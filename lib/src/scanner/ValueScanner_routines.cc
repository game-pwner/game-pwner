...
//todo[low] 0905 fit below in templates if possible, inline any macro
/* for convenience */
#define SCAN_ROUTINE_ARGUMENTS (const PWNER::mem64_t *memory_ptr, size_t memlength, const PWNER::value_t *old_value, const PWNER::Cuservalue *user_value, PWNER::flag& saveflags)
unsigned int (*PWNER::sm_scan_routine) SCAN_ROUTINE_ARGUMENTS;

#define MEMORY_COMP(value,field,op)  ((value)->flags & PWNER::flag_t::flag_##field && (memory_ptr-> field) op (value)-> field)
...
PWNER::scan_routine_t PWNER::sm_get_scanroutine(PWNER::Edata_type dt, PWNER::Ematch_type mt, const flag& uflags, bool reverse_endianness)
{
...
bool
PWNER::sm_choose_scanroutine(PWNER::Edata_type dt, PWNER::Ematch_type mt, const PWNER::Cuservalue* uval, bool reverse_endianness)
{
    PWNER::flag uflags = uval ? uval->flags : PWNER::flag_t::flags_empty;

    /* Check scans that need an uservalue */
    if (mt == PWNER::Ematch_type::MATCHEQUALTO
    ||  mt == PWNER::Ematch_type::MATCHNOTEQUALTO
    ||  mt == PWNER::Ematch_type::MATCHGREATERTHAN
    ||  mt == PWNER::Ematch_type::MATCHLESSTHAN
    ||  mt == PWNER::Ematch_type::MATCHRANGE
    ||  mt == PWNER::Ematch_type::MATCHINCREASEDBY
    ||  mt == PWNER::Ematch_type::MATCHDECREASEDBY)
    {
        ...
        PWNER::flag possible_flags = PWNER::flag(dt);
        if ((possible_flags & uflags) == PWNER::flag_t::flags_empty) {
            /* There's no possibility to have a match, just abort */
            PWNER::sm_scan_routine = nullptr;
            return false;
        }
    }

    PWNER::sm_scan_routine = sm_get_scanroutine(dt, mt, uflags, reverse_endianness);
    return (PWNER::sm_scan_routine != nullptr);
}
