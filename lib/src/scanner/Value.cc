                           ...
                           std::vector<PWNER::Cuservalue>& uservalue)
{
    ...
    switch (data_type) {
        case PWNER::Edata_type::BYTEARRAY:
//            if (!parse_uservalue_bytearray(text, &uservalue[0])) {
//                return false;
//            }
            return;
        case PWNER::Edata_type::STRING:
        ...
        default:
            ...
            auto boilerplate_a = [&](const string &MASK, PWNER::Ematch_type MATCH_TYPE_NO_VALUE) -> bool
            {
            ...
            if (boilerplate_a("++", PWNER::Ematch_type::MATCHINCREASED)) goto valid_number;  // a2
            if (boilerplate_a("--", PWNER::Ematch_type::MATCHDECREASED)) goto valid_number;  // a3

            auto boilerplate_b = [&](const string &MASK, PWNER::Ematch_type MATCH_TYPE, PWNER::Ematch_type MATCH_TYPE_NO_VALUE) -> bool
            {
                size_t cursor = pattern.find(MASK);
                if (cursor == string::npos)  // if mask not found
                    return false;
            ...
            if (boilerplate_b("!=", PWNER::Ematch_type::MATCHNOTEQUALTO,  PWNER::Ematch_type::MATCHCHANGED))    goto valid_number; // 2
            if (boilerplate_b("+=", PWNER::Ematch_type::MATCHINCREASEDBY, PWNER::Ematch_type::MATCHINCREASED))  goto valid_number; // 3
            if (boilerplate_b("-=", PWNER::Ematch_type::MATCHDECREASEDBY, PWNER::Ematch_type::MATCHDECREASED))  goto valid_number; // 4
            if (boilerplate_b(">",  PWNER::Ematch_type::MATCHGREATERTHAN, PWNER::Ematch_type::MATCHINCREASED))  goto valid_number; // 5
            if (boilerplate_b("<",  PWNER::Ematch_type::MATCHLESSTHAN,    PWNER::Ematch_type::MATCHDECREASED))  goto valid_number; // 6


            // todo[low] MATCHNOTINRANGE "!= 0..1", MATCHNOTINRANGE / MATCHEXCLUDE
            auto boilerplate_c = [&](const string &MASK, PWNER::Ematch_type MATCH_TYPE) -> bool
            {
                size_t cursor = pattern.find(MASK);
                if (cursor == string::npos)   // if mask not found
                    return false;
